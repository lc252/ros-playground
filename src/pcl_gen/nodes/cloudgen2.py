...
def magic_carpet(n):
    ...
    z = np.sin(x + 2*np.pi*0.05*time.time())*0.5 + 0.5# + np.sin(y * 2*np.pi*0.25 + time.time())*0.25 
...
def two_planes(n):
    ...
    # x and y points do not change
    x = np.array([np.linspace(0,1,25) for _ in range(25)])
    ...
    z = np.concatenate((np.zeros(25*25), np.ones(25*25)))
...
def cloudgen():
    ...
    pub = rospy.Publisher("raw_points", PointCloud2, queue_size=1)
